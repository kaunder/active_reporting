        ...
        ActiveRecord::Base.connection.execute(
          <<-SQL
            ...
            BEGIN
              ...
              IF field IN ('week') THEN RETURN STR_TO_DATE(CONCAT(YEARWEEK(source, 3), ' Monday'), '%x%v %W'); END IF;
              ...
              IF field IN ('quarter') THEN SET source = source - INTERVAL CASE WHEN MONTH(source) % 3 > 0 THEN MONTH(source) % 3 - 1 ELSE 2 END MONTH; END IF;
              ...
              IF field IN ('decade') THEN SET source = source - INTERVAL YEAR(source) % 10 YEAR; END IF;
